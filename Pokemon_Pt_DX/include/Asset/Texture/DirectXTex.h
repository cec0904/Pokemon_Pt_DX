...
namespace DirectX
{
    ...
    DIRECTX_TEX_API HRESULT __cdecl GetMetadataFromWICMemory(
        ...
        _Out_ TexMetadata& metadata,
        _In_ function<void __cdecl(IWICMetadataQueryReader*)> getMQR = nullptr);
    ...
    DIRECTX_TEX_API HRESULT __cdecl GetMetadataFromDDSMemory(
        _In_reads_bytes_(size) const byte* pSource, _In_ size_t size,
        _In_ DDS_FLAGS flags,
        _Out_ TexMetadata& metadata) noexcept;
    DIRECTX_TEX_API HRESULT __cdecl GetMetadataFromDDSMemoryEx(
        _In_reads_bytes_(size) const byte* pSource, _In_ size_t size,
    ...
    public:
        ...
        Blob(Blob&& moveFrom) noexcept : m_buffer(nullptr), m_size(0) { *this = move(moveFrom); }
        ~Blob() { Release(); }
    ...
    DIRECTX_TEX_API HRESULT __cdecl SaveToWICMemory(
        ...
        _In_ function<void __cdecl(IPropertyBag2*)> setCustomProps = nullptr);
    DIRECTX_TEX_API HRESULT __cdecl SaveToWICMemory(
    ...
    DIRECTX_TEX_API HRESULT __cdecl ConvertEx(
        ...
        _In_ function<bool __cdecl(size_t, size_t)> statusCallBack = nullptr);
    DIRECTX_TEX_API HRESULT __cdecl ConvertEx(
        _In_reads_(nimages) const Image* srcImages, _In_ size_t nimages, _In_ const TexMetadata& metadata,
    ...
    DIRECTX_TEX_API HRESULT __cdecl EvaluateImage(
        _In_ const Image& image,
        _In_ function<void __cdecl(_In_reads_(width) const XMVECTOR* pixels, size_t width, size_t y)> pixelFunc);
    DIRECTX_TEX_API HRESULT __cdecl EvaluateImage(
        _In_reads_(nimages) const Image* images, _In_ size_t nimages, _In_ const TexMetadata& metadata,
        _In_ function<void __cdecl(_In_reads_(width) const XMVECTOR* pixels, size_t width, size_t y)> pixelFunc);
    ...
    DIRECTX_TEX_API HRESULT __cdecl TransformImage(
        _In_ const Image& image,
        _In_ function<void __cdecl(_Out_writes_(width) XMVECTOR* outPixels,
    ...
    DIRECTX_TEX_API HRESULT __cdecl EncodeDDSHeader(
        _In_ const TexMetadata& metadata, DDS_FLAGS flags,
        _Out_writes_bytes_to_opt_(maxsize, required) byte* pDestination, _In_ size_t maxsize,
        _Out_ size_t& required) noexcept;

    DIRECTX_TEX_API HRESULT __cdecl EncodeDDSHeader(
        _In_ const TexMetadata& metadata, DDS_FLAGS flags,
        _Reserved_ nullptr_t, _In_ size_t maxsize,
        _Out_ size_t& required) noexcept;
#endif
